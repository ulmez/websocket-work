...
public class Data {
	...
	public static WhiteboardConvert convertedWhiteboard(Whiteboard w) {
		...
		for(int i = 0; i < w.getDb().size(); i++) {
			...
			PostItConvert pit1 = new PostItConvert(id, title, information, color);
			
			pits1.add(pit1);
		}
		
		WhiteboardConvert wbt1 = new WhiteboardConvert(w.getWhiteboard(), pits1, w.getId());
	...
	public static List<WhiteboardConvert> convertedAllWhiteboards() {
		...
		for(int i = 0; i < whiteboards.size(); i++) {
			Whiteboard wb1 = (Whiteboard) session.get(Whiteboard.class, ((Whiteboard) whiteboards.get(i)).getId());
	...
	public static WhiteboardConvert convertedWhiteboard(String whiteboardName) {
		...
		Query query = session.createQuery("SELECT w FROM Whiteboard w WHERE w.whiteboard=" + "'" + whiteboardName + "'");
		List<?> whiteboards = query.list();
